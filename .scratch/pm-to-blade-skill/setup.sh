...
#     PI_DS_REPO_PATH = Pi-Design-System repo 根目錄（含 src/ fonts/ assets/）
#
...
PI_SRC="${1:-}"
if [[ -z "$PI_SRC" ]]; then
  echo "❌ 缺參數：Pi DS repo 路徑" >&2
  echo "   用法：bash setup.sh <PI_DS_REPO_PATH>" >&2
  exit 1
fi
PI_SRC="${PI_SRC%/}"                       # 去尾斜線
...
for f in symicon-6.4s.woff2 symicon-6.4s.woff; do
  if [[ -f "public/fonts/$f" ]]; then
    ...
    note "找不到 $PI_SRC/fonts/$f（若不用 icon 可忽略）"
  fi
done
