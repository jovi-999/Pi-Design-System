...
需求元件在 vendored Pi DS 找不到現成件時走這條。component 與 token 共用同一 loop，提議文字註明類別。搭配 [token-rules.md](token-rules.md) 一起看。

## 三層門檻（判斷是否真缺件）

1. **能用現成 component class 直接命中** → 直接用，不提議。
2. **不能，但用現有 token（border / shadow / spacing / color / radius / type）疊得出視覺** → **組合件**：prototype 直接做（Blade 標記 + prototype-scoped scss，值全用現有 token），handoff 標 `⚠️ 待確認` 並註明「組合而非正式元件」，**不進提議 loop**。
3. **兩者皆不能**（需 JS 行為或全新視覺 primitive，例：日曆彈窗、可排序表格、自動輪播 carousel）→ **真缺件**，進提議 loop。
...
2. **否決 → 改用現有** → PM 指定現有件替代 → 退回門檻 1/2，prototype 直接用該件，不再是缺件。
3. **擱置** → prototype 留空位 + 一句 TODO，不進 handoff 正式清單，記在兩份 handoff 底部「擱置項」。

**無回應時 AI 不自作主張，停等 PM。**

## prototype placeholder（可跑，零自創）

- 用最接近的現有件當殼（Blade 標記 + 現有 `gl_*` class）。
- 外層包 `gl_border-outer`（現有 solid 描邊）框出範圍。
- 可見文字標籤「🆕 xxx（待確認）」+ Blade 註解 `{{-- 暫用 X 頂替，正式件待前端做 --}}`。
- **無 dashed border token**（查 vendored `pi-ds/` 確認）；禁自創，故用現有 solid `gl_border-outer`。
