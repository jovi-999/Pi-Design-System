...
| **navbar 本體** | 無正式元件 —— **組合件** | 用現有 token（`--cl-basic-900` 底、`--radius-md`）+ 純 HTML flex 疊出，**⚠️ 待確認：組合而非正式元件**（04 門檻第 2 層，未進提議 loop） |

## 狀態 + 互動（AI 推測）

- input 空值/error/disabled 樣式：`.gl_form-control` 內建 `:focus` / `[disabled]`；error 需搭 `.is-invalid .form-feedback` — ⚠️ 待確認
- 送出鈕 loading / 送出後狀態 — ⚠️ 待確認
- 表單驗證觸發時機（blur / submit）— ⚠️ 待確認

## 待確認新件

- 無真缺件（navbar 以組合件處理，見元件清單）。
